    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onClose}>
      ...
      >
        ...
        <div className="flex flex-1 min-h-0">
            ...
            <div className="w-44 shrink-0 border-r border-gray-100 overflow-y-auto py-2">
              ...
              {categorized.map((cat, i) => {
                ...
                return (
                  <div key={cat.key}>
                    {showMaterialHeader && (
                      <div className="px-4 pt-2 pb-1 text-[10px] font-semibold text-gray-300 uppercase tracking-wider border-t border-gray-100 mt-1">Material</div>
                    )}
                    <button
                      type="button"
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {searchResults ? (
              searchResults.flat ? (
                <>
                  <p className="text-xs text-gray-400 mb-3">{searchResults.flat.length} results for &ldquo;{search}&rdquo;</p>
                  <IconGrid icons={searchResults.flat} value={value} onChange={onChange} onClose={onClose} />
                </>
              ) : (
                <>
                  <p className="text-xs text-gray-400 mb-3">{searchResults.total} results for &ldquo;{search}&rdquo;</p>
                  {searchResults.lucide.length > 0 && (
                    <>
                      <div className="text-[10px] font-semibold text-gray-300 uppercase tracking-wider mb-2">Lucide ({searchResults.lucide.length})</div>
                      <IconGrid icons={searchResults.lucide} value={value} onChange={onChange} onClose={onClose} />
                    </>
                  )}
                  {searchResults.material.length > 0 && (
                    <>
                      <div className={`text-[10px] font-semibold text-gray-300 uppercase tracking-wider mb-2 ${searchResults.lucide.length > 0 ? 'mt-4 pt-3 border-t border-gray-100' : ''}`}>Material ({searchResults.material.length})</div>
        ...
        <div className="px-5 py-2.5 border-t border-gray-100 flex items-center justify-between">
          <span className="text-[11px] text-gray-300">{allIcons.length} icons available</span>
